...
class ChannelAttention(nn.Module):
    ...
    This module implements channel attention by aggregating spatial information 
    through both average and max pooling, then using a shared MLP to compute 
    ...
    Example:
        ...
        >>> x = torch.randn(1, 64, 32, 32)
        >>> attention_weights = channel_attn(x)
...
class SpatialAttention(nn.Module):
    ...
    This module implements spatial attention by aggregating channel information 
    through both average and max pooling along the channel dimension, then using 
    a convolutional layer to compute spatial attention weights.
    ...
    Example:
        ...
        >>> x = torch.randn(1, 64, 32, 32)
        >>> attention_weights = spatial_attn(x)

    Shape:
        - Input: (B, C, H, W)
        - Output: (B, 1, H, W) [spatial attention weights]
    """
    ...
    def forward(self, x):
        ...
        """
        # Max & Avg Pool across channel dimension
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        Applies channel attention followed by spatial attention to the input tensor.

        Args:
            x (torch.Tensor): Input tensor of shape (B, C, H, W).

        Returns:
            torch.Tensor: Output tensor with attention applied, same shape as input (B, C, H, W).
        """
        x = x * self.channel_attention(x)  # broadcast along H, W
        x = x * self.spatial_attention(x)  # broadcast along C
        return x
